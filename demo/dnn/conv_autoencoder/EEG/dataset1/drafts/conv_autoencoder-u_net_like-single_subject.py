...
class dataset(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self, data, subj=0):
        '''
        Inputs:
            data - numpy array or tensor of shape (subj,ims, chans, times)
            subj - number of subject whcih data to return
        '''
        self.subj = subj
    ...
    def __getitem__(self, idx):
        return self.data[idx, self.subj].type(torch.float32).unsqueeze(0)
...
if __name__=='__main__':
    ...
    model = conv_autoencoder(n_subj = 1, ch_time = ch_time,\
    ...
    for epoch in range(args.n_epochs):
        ...
        if cntr_epta % args.epta == 0 and cntr_epta !=0:
            ...
            im1=ax[0].imshow(batch[0,0,:,:])
            im2=ax[1].imshow(dec[0,0,:,:].detach())
            figs.append(fig)
