...
#SBATCH --job-name=hpar_grid-100hz-v2
...
n_epochs=800
bpl=50
dataset_dir="/scratch/akitaitsev/intersubject_generalization/linear/dataset1/dataset_matrices/100hz/time_window26-80/"
out_dir="/scratch/akitaitsev/intersubject_generalization/dnn/perceiver/EEG/dataset1/leftthomas/projection_head/100hz/hpar-grid-ver1/"
...
perc_latent_head=(1 6)
perc_depth=(1 6)
out_dim_ENC=(100 200) # as if mvica with PCA 200
out_dim_PH=(100 400) # as if mvica with PCA 100
...
python perceiver-projection_head-eeg-leftthomas.py -gpu -batch_size $batch_size -out_dir $out_dirs -lr $lr -bpl 50 -n_epochs $n_epochs -perc_latent_array_dim $perc_latent_array_dims -perc_num_latent_dim $perc_num_latent_dims -perc_latent_heads $perc_latent_heads -perc_depth $perc_depths -out_dim_ENC $out_dim_ENCs -out_dim_PH $out_dim_PHs -eeg_dir $dataset_dir
