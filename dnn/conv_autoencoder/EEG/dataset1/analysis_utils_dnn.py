...
def _mean_max_acc_over_shuffles(step_dir, nshuffles, net='conv_autoencoder'):
    '''
    ...
    Inputs:
        ...
        net - str, which net was used conv_autoencoder, perceiver or resnet. 
            (Default== conv_autoencoder).
    ...
    # load data
    if net=='conv_autoencoder':
        dec_name = 'decoder'
    elif net=='perceiver' or net=='resnet':
        dec_name = 'projection_head'

    for sh in range(nshuffles):
        accs = joblib.load(step_dir.joinpath(('seed'+str(sh)), \
        'test_accuracies.pkl'))
        enc_av.append(accs["encoder"]["average"])
        dec_av.append(accs[dec_name]["average"])
        dec_sw_mean.append(accs[dec_name]["subjectwise"]["mean"])
        dec_sw_sd.append(accs[dec_name]["subjectwise"]["SD"])
...
def create_saturation_profile_data(method_dir, nshuffles, steps, net):
    '''
    ...
    for step in steps:
        enc_av_tmp, enc_sw_tmp, dec_av_tmp, dec_sw_tmp = _mean_max_acc_over_shuffles(\
            method_dir.joinpath(('step'+str(step))), nshuffles, net=net)
...
def plot_saturation_profile(enc_av, enc_sw, dec_av, dec_sw, labels, layer='encoder', \
