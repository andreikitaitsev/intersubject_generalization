...
def ContrastiveLoss_leftthomas(out1, out2, temperature, normalize="normalize"):
    '''Inputs:
...
if __name__=='__main__':
    ...
    # create datasets
    dataset_train = eeg_dataset_train(Y_tr_incr, net='perceiver')
    dataset_test = eeg_dataset_test(data_test, net='perceiver')
    dataset_train_for_assessment = eeg_dataset_test(Y_tr_incr, net='perceiver')
    ...
    for epoch in range(n_epochs):
        ...
        if cntr_epta % epta == 0:
            ...
            eeg_train_projected_ENC = project_eeg(model, train_dataloader_for_assessment, layer="encoder", split_size=30) 
            eeg_test_projected_ENC = project_eeg(model, test_dataloader, layer = "encoder")  

            # treat projection head output as EEG
            eeg_train_projected_PH = project_eeg(model, train_dataloader_for_assessment, split_size=30) 
            eeg_test_projected_PH = project_eeg(model, test_dataloader)  
            
            av_ENC, sw_ENC = assess_eeg(X_tr_incr, X_test, eeg_train_projected_ENC, eeg_test_projected_ENC)
            av_PH, sw_PH = assess_eeg(X_tr_incr, X_test, eeg_train_projected_PH, eeg_test_projected_PH)
