...
if __name__=='__main__':
    ...
    cntr_epta=0 
    net_states_epta=[] # best net state every epoch
    losses_epta=[]
    net_states_global=[]
    ...
    for epoch in range(n_epochs):
        ...
        for batch1, batch2 in train_dataloader:
            ...
            # save state dict for every optimization step
            if args.pick_best_net_state:
                net_states_local.append(copy.deepcopy(model.state_dict()) )
        ...
        # pick up the net state with min loss across each epoch 
        if args.pick_best_net_state:
            net_states_epta.append(net_states_local[ np.argmin(losses["epoch"+str(epoch)]) ] )
            losses_epta.append(np.min(losses["epoch"+str(epoch)] ) )
        ...
        if cntr_epta % epta == 0:
            ...
            # pick up model state with min loss each epta epochs
            net_states_global.append(net_states_epta[np.argmin(losses_epta)] )
            net_states_epta=[]
            losses_epta=[]
    ...
    if args.pick_best_net_state:
        best_net_state = net_states_global[ np.argmax(accuracies["encoder"]["average"]) ]
