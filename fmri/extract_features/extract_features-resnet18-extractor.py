...
def extract_features_resnet18(video_list, model, train_test_split=True, train_last_idx=1000, \
    debug=False):
    '''
    Extract intermediate features of resnet18. 
    ...
    def _extract_features_single_video_multiple_layers(ims, model, layers=None):
        ...
        # torchextractor acesses the last sub/sub-sub layer of each given layer.
        if layers is None:
        ...
        out, layer_feats = model(ims)
        
        # average activations over frames for each layer
        for layer in layers:
            layer_feats[layer] = layer_feats[layer].cpu().numpy()
            acts.append(np.mean(np.reshape(layer_feats[layer], (layer_feats[layer].shape[0],-1)),axis=0))
...
class feature_extractor(object):
    ...
    Methods:
        ...
        '''
    def __init__(self, model, feature_extraction_function=extract_features_resnet18,\
        postprocessor=None):
...
if __name__ == '__main__':
    ...
    feature_extractor = feature_extractor(model, extract_features_resnet18, postprocessor)
