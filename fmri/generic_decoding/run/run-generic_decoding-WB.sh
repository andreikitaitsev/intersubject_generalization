...
out_base='/scratch/akitaitsev/fMRI_Algonautus/generic_decoding/full_track/'
real_base="/scratch/akitaitsev/fMRI_Algonautus/intersubject_generalization/full_track/"
pred_base="/scratch/akitaitsev/fMRI_Algonautus/regression/full_track/"
...
do
    ...
    do
        out_dirs[$ind]=$out_base"/PCA"$n_comp"/"
        real_files[$ind]=$real_base"/PCA"$n_comp"/WB_shared_test.pkl"
        pred_files[$ind]=$pred_base"/PCA"$n_comp"/Y_test_pred_"$dtype".pkl"
        dtypes[$ind]=$dtype
        n_comps[$ind]=$n_comp
        ((ind=ind+1))
   done
