...
'''
__all__ = ['eigPCA', 'intersubject_generalizer']
...
def eigPCA(data, n_comp=None):
    """
    PCA via eigendecomposition of the covariance matrix.
    Inputs: 
        data - 2d numpy array of shape (n_samples, n_features)
        n_comp - number of comps to retain in the data. If none, leaves as
            many components as there are features in the original data
    Outputs:
        pc_data - 2d np array of transformed data
        evals - sorted and trunctated eigenvalues
        evecs - ..-.. eigenvectors
    """
    m, n = data.shape
    ...
    if n_comp is None: 
        n_comp=len(evals)
    evecs = evecs[:, :n_comp]
    pc_data = np.dot(evecs.T, data.T).T
    return pc_data, evals, evecs
    ...
    def __init__(self, method, n_components, dimension_reduction, method_kwargs=None):
        ...
        Attributes:
            ...
            P - dimensionality reduction matrix for every subject(pca or srm,
                see multiview ica funcs). If none, returns identity matrix
...
if __name__=='__main__':
    ...
    parser.add_argument('-no_val', type=bool, default=True, help='Whether not to use val set. Default =True '
    '(no val set assumed.')
