...
def sliding_window_control(dataset_train, dataset_val, dataset_test, window_length, \
    hop_length=None):
    '''
    Sliding window intersubject geenralizaion. Generalizer object is fit on time window of
    train dataset and used to predict val and test datasets in the same window.
    ...
    for inds in ind_list:
        ...
        windowed_val.append(featuremat_val)
    metadata = {'hop_length': hop_length, 'window_length': window_length,
        'index_list': ind_list}
    return windowed_train, windowed_val, windowed_test, metadata
...
if __name__=='__main__':
    ...
    if args.method != 'control':
        ...
        windowed_train, windowed_val, windowed_test, metadata=sliding_window_control(dataset_train, \
            dataset_val, dataset_test, args.window_length, args.hop_length)

        names=['_train.pkl', '_val.pkl', '_test.pkl']
        for sh_mat, name in zip([windowed_train, windowed_val, windowed_test], names):
            joblib.dump(sh_mat, output_dir.joinpath(('shared'+name))) 
        joblib.dump(metadata, output_dir.joinpath('metadata.pkl'))
