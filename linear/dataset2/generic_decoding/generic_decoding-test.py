...
def _generic_decoding_cv(real_data, predicted_data, is_raw=False):
    '''
    ...
    Inputs:
        ...
        is_raw - bool, whether the rel data is a featurematrix (raw EEG)
    Outputs:
        cor_mat_cum
        gen_dec_cum
    '''
    def _cv_split_real_data(real_data, is_raw):
        '''
        Split real EEG data into the list of leave-one-out CV folds.
        Inputs:
            real_data - 3d np array - featurematrix (if is_raw=True), (subj, feat, ims)
                or shared_test (subj, ims, feat) (if is_raw=False)
            is_raw - bool, whether the input data is a raw EEG untransformed by IGA (control)
                or is a shared space response transformed by IGA (mvica, permica, etc.).
        Outputs:
        ...
        CV = LeaveOneOut()
        split_data=[]
        ids = np.linspace(0, real_data.shape[0], real_data.shape[0], endpoint=False, dtype=int)
        for tr, te in CV.split(ids):
            # subject, whos data was not used to train the regression
            split_data.append(np.mean(real_data, axis=0) )
        return split_data
            
    # split real data accoring to CV used in regression to get the predicted data
    real_data = _cv_split_real_data(real_data, is_raw)
...
def generic_decoding(real_data, pred_data, regr_type, sliding_window, cv, is_raw=False):
    '''
    ...
    Flags - for data comming from the different types of analysis:
        sliding_window 
        cv
        is_raw (Default=False) - flag only meaningful for cv. If the analysis
        is performed on the control (non-transformed raw EEG)
    Outputs:
    ...
    if sliding_window and not cv:
        ...
        cor, res = _generic_decoding_cv(real_data, pred_data, is_raw)
...
if __name__ =='__main__':
    ...
    parser.add_argument('-is_raw', action='store_true', default=False, help=
    'Bool flag (only meaningful for cv), whether the analysis is performed on the control'
    '(non-transformed raw EEG). Default=False.')
