...
import sys
sys.path.append('/home/akitaitsev/code/intersubject_generalization/linear/dataset1/create_featurematrices')
...
def sliding_window(generalizer, dataset_train, dataset_test, window_length, \
    hop_length=None):
    '''
    Sliding window intersubject geenralizaion. Generalizer object is fit on time window of
    train dataset and used to predict and test datasets in the same window.
    ...
    Outputs:
        projected_data - list of projected train and test data (each of of the entries
                         is a list of (shape n_windows +1))
        backprojected_data -  list of projected train and test data
    ...
    metadata = {'hop_length': hop_length, 'window_length': window_length,
        'index_list': ind_list}
    projected_data = [shared_train, shared_test]
    backprojected_data = [backproject_train, backproject_test]
    
    return projected_data, backprojected_data, generalizers, metadata


def sliding_window_control(dataset_train, dataset_test, window_length, \
    hop_length=None):
    '''
    Sliding window intersubject geenralizaion. Generalizer object is fit on time window of
    train dataset and used to predict and test datasets in the same window.
    ...
    for inds in ind_list:
        ...
        windowed_train.append(featuremat_train)
        windowed_test.append(featuremat_test)
    metadata = {'hop_length': hop_length, 'window_length': window_length,
        'index_list': ind_list}
    return windowed_train, windowed_test, metadata
...
if __name__=='__main__':
    ...
    'and then used to project and backproject same time window of test data.')
    ...
    'generalization (multiviewica, permica, groupica, contorl).', default='multiviewica')
    ...
    if args.method != 'control':
        ...
        projected_data, backprojected_data, generalizers, metadata=sliding_window(\
            generalizer, dataset_train, dataset_test, args.window_length, args.hop_length)

        
        names=['_train.pkl', '_test.pkl']
        ...
        windowed_train, windowed_test, metadata=sliding_window_control(dataset_train, \
            dataset_test, args.window_length, args.hop_length)

        names=['_train.pkl', '_test.pkl']
        for sh_mat, name in zip([windowed_train, windowed_test], names):
            joblib.dump(sh_mat, output_dir.joinpath(('shared'+name))) 
        joblib.dump(metadata, output_dir.joinpath('metadata.pkl'))
